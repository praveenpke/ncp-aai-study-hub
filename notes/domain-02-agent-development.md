...
|---|---|---|
| **State** | Shared, typed dict (TypedDict/Pydantic) flowing through the graph | **Reducers** define how a node's return *merges* into existing state. Default = **overwrite** (new value replaces old); annotate a field with a reducer to change that — e.g. `messages: Annotated[list, add_messages]` **appends** new messages instead of clobbering the history. Forgetting the reducer is why a chat node "loses" prior turns |
...
| **Checkpointer** | Persistence plugged in at compile: `compile(checkpointer=...)` — `InMemorySaver` (dev), `SqliteSaver`/`PostgresSaver` (prod) | Saves state after every **super-step** per **`thread_id`** (passed in `config={"configurable": {"thread_id": ...}}`). A *super-step* is one round of the graph in which every currently-active node runs (so parallel branches execute and checkpoint together as one atomic unit, à la Pregel/BSP) — that's the granularity resume/time-travel rewind to. Enables resume-after-crash, multi-turn memory, time travel |
...
**The prebuilt agent factory (LangChain/LangGraph 1.0, Oct 2025):** for the *common* "model + tools in a loop" case you don't hand-build the graph — you call a factory that returns a compiled LangGraph. As of **LangChain/LangGraph 1.x** the canonical factory is **`from langchain.agents import create_agent`** (runs on LangGraph, adds a middleware system). The older **`from langgraph.prebuilt import create_react_agent`** is **deprecated** (still works with a warning; slated for removal in v2.0) — if you see `create_react_agent` as the "current" answer, it's a pre-1.0 artifact. Both compile down to a graph, so checkpointers/interrupts/streaming below apply unchanged; you only drop to raw `StateGraph` when you need custom nodes/edges the factory doesn't express.
...
- **Vision-language models (VLMs):** image + text in, text out. NIM VLM endpoints follow the OpenAI spec — a user message whose `content` is a list of parts: `{"type": "image_url", "image_url": {"url": ...}}` where the URL is either a web URL or a **base64 data URI** (`data:image/png;base64,...`; JPG/JPEG/PNG). NVIDIA guidance: **put the image part before the text part**. Hosted build.nvidia.com endpoints cap inline base64 payloads (~180 KB); larger files go through the assets upload API. Models to recognize: `meta/llama-3.2-90b-vision-instruct`, NVIDIA's Nemotron Nano VL line (document-intelligence focused). **Current (2026):** **Nemotron 3 Nano Omni** (April 2026) is NVIDIA's headline open multimodal model — one ~30B-total/~3B-active hybrid-MoE that takes **text + image + video + audio** in a single model, superseding Nemotron Nano VL V2; it's the model to name when a question asks for "one model handling documents, audio, and video."
- **Document understanding:** two routes — (a) VLM reads page images directly (layout-aware: tables, charts, scanned forms; the specialty of the Nemotron Nano VL line / Nemotron 3 Nano Omni, also the basis of OCR-free "visual retrieval"), or (b) extraction pipeline (NeMo Retriever extraction NIMs parse PDFs into text/tables/images, then embed). VLM route preserves layout semantics; pipeline route scales cheaper for bulk text.
...
**Tool Calling Agent.** `_type: tool_calling_agent` is the agent type the exam centers on: it uses the model's **native function-calling** (the §3.3 OpenAI tool spec NIM serves) rather than ReAct-style text parsing, so it's the right pick when the model supports tool calls and you want **reliable, schema-true tool invocation**. Its knobs: `tool_names` (functions/function-groups it may call), `llm_name`, `verbose` (default `False`), `handle_tool_errors` (default `True` — catches a tool exception and feeds it back as a `ToolMessage` so the agent self-corrects, the §3.3 error-as-tool-result pattern built in), `max_iterations` (default `15`), and **`description`** (default `"Tool Calling Agent Workflow"`) — the latter sets the tool description used when this agent is nested *as a tool inside another agent*, which is how NAT composes **multi-agent** hierarchies from the same YAML. Note the agent's *final answer* is plain text by default — the tool-calling guarantee covers the **arguments** the model emits for each tool call (schema-true tool invocation), not the answer; for a typed final object, pair the agent with §3.2 guided decoding / a Pydantic gate on its output, or have a structured tool return the object. (NAT also ships `react_agent`, `reasoning_agent`, and `rewoo_agent` workflow types; Tool Calling is the default for function-capable models.)
...
- MCP specification & primitives: https://modelcontextprotocol.io/specification/2025-06-18 ; Linux Foundation donation: https://www.linuxfoundation.org/press/linux-foundation-announces-the-formation-of-the-agentic-ai-foundation
...
*What to notice:* same client, same endpoint — multimodality is just a content-parts list instead of a string (trap #14): `image_url` accepts a web URL or a base64 *data URI*, and the image part precedes the text part. Swap the model name for a Nemotron Nano VL / Nemotron 3 Nano Omni when the task is dense document layout (or needs audio/video too).
...
6. **MCP's institutionalization vs the skeptics (Dec 2025 →)** — MCP was donated (Dec 9, 2025) to the new **Agentic AI Foundation (AAIF)** under the Linux Foundation — anchored alongside Block's *goose* and OpenAI's *AGENTS.md* — and hit ~97M monthly SDK downloads by early 2026, with every major provider on board — the "USB-C of tools" bet effectively won the protocol war. The counter-discourse hasn't quieted: practitioners flag tool-description bloat eating context budgets (Anthropic's own guidance: keep 3-5 tools loaded, discover the rest dynamically), unvetted-server supply-chain risk, and the argument that for code-capable agents, a script or CLI beats a tool catalog. The mature 2026 position: MCP for *integration breadth*, curated native tools for the *hot path*. https://www.linuxfoundation.org/press/linux-foundation-announces-the-formation-of-the-agentic-ai-foundation ; https://www.thoughtworks.com/en-us/insights/blog/generative-ai/model-context-protocol-mcp-impact-2025
