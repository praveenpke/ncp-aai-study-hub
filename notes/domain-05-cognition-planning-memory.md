...
- **ReWOO variant:** plan all steps upfront *with variable placeholders* (`#E1`, `#E2`) so evidence from step 1 substitutes into step 3 without re-calling the planner LLM each step — cuts token cost vs ReAct. *Concretely:* the planner emits something like `Plan: find the CEO. #E1 = Search["Acme CEO"]` then `Plan: find their birth year. #E2 = Search["#E1 birth year"]` — at run time the Worker runs `#E1`, captures the literal result (e.g. "Dana Lee"), and textually substitutes it into `#E2`'s query (`Search["Dana Lee birth year"]`) before executing — no LLM call needed to chain step 1's output into step 2.
...
| **NeMo Retriever embedding NIMs** | The recall half of vector-store memory | The same embedding NIMs from the RAG stack (e.g. `llama-nemotron-embed-1b-v2` — renamed from `llama-3.2-nv-embedqa-1b-v2` in NeMo Retriever NIM 1.13.0) embed memories for semantic search — long-term memory *is* a retrieval pipeline over your own past |
...
> **Currency note — Nemotron 3 is the 2026 headline reasoning family.** The toggle *mechanism* above (post-train one checkpoint on `detailed thinking on/off`) is unchanged and still exam-canonical, but the model lineup moved on. **Nemotron 3** debuted Dec 2025 (Nano first) and expanded through H1 2026 as hybrid **Mamba-Transformer latent-MoE** models built for agentic reasoning/planning: **Nemotron 3 Nano** (~30B total / ~3B active), **Nemotron 3 Super** (120B total / 12B active, announced Mar 2026), **Nemotron 3 Ultra** (~500–550B total / ~50–55B active — NVIDIA's largest open model, positioned explicitly for reasoning/planning/agentic, shown at Computex June 2026), plus **Nemotron 3 Nano Omni** (multimodal). Reasoning control evolved too: Nemotron 3 **Super** switches with `/think` and `/no_think` directives (same inverted default as Llama Nemotron Super v1.5 — reasoning ON unless you `/no_think`); **Nano** gates reasoning with an `LLM_ENABLE_THINKING` flag plus a configurable **thinking budget** (`LLM_REASONING_BUDGET` / `LLM_MAX_THINKING_TOKENS`) that caps chain-of-thought tokens to keep latency/cost predictable — the productized answer to the "overthinking" failure mode (trap #12). At the **API level** the same toggle is `chat_template_kwargs={"enable_thinking": True/False}` with an optional `"reasoning_budget"` (plus a `LLM_LOW_EFFORT` short-reasoning mode) — and note the *default when nothing is specified is reasoning ON*, so disabling is the explicit action. (Verified June 2026: Nemotron 3 Nano is 30B total / 3.5B active, 23 Mamba-2+MoE layers + 6 attention; Super is 120B / 12B active LatentMoE, ~5x throughput vs Nemotron 2 Super.) The Llama Nemotron v1.x family (LN-Nano 8B / LN-Super 49B / LN-Ultra 253B) below is still the foundational reasoning family the exam's `detailed thinking on/off` questions are written against; treat Nemotron 3 as the current generation layered on the same toggle idea.
...
> **Name & package currency (2026):** "NAT" = **NeMo Agent Toolkit, current major 1.7.x** — formerly the **Agent Intelligence (AIQ) Toolkit**. The pip package is now **`nvidia-nat`** (the transitional `aiqtoolkit`/`agentiq` packages were removed in 1.7) and the import namespace moved **`aiq` → `nat`**. That rename is *why* the TTC module path is `nat.experimental.test_time_compute` and every plugin below is `nvidia-nat-*`. If an older doc or a distractor says "AIQ Toolkit," `import aiq.*`, or `pip install aiqtoolkit`, that's the pre-rename label for the same product — don't treat it as a different toolkit.
...
11. **Reasoning toggle direction** — Llama Nemotron v1: `"detailed thinking on"`/`"off"` in the **system prompt** (first message). Super 49B **v1.5 inverts it**: reasoning on by default, `/no_think` disables (Nemotron 3 Super carries the same `/think`·`/no_think` convention). Also: thinking-on wants temp 0.6/top_p 0.95, thinking-off wants greedy — backwards from intuition ("more deterministic for reasoning") and a likely distractor. Newer Nemotron 3 Nano adds a **configurable thinking budget** (`LLM_ENABLE_THINKING` + `LLM_REASONING_BUDGET`/`LLM_MAX_THINKING_TOKENS`) to cap chain-of-thought tokens — don't confuse the *budget knob* with the on/off *toggle*.
...
*What to notice:* compaction replaces the *older* history with a maintained `RunningSummary` while recent turns stay verbatim — the §3.4 pattern. The three similarly-named knobs do different jobs: `max_tokens_before_summary` is the *trigger* (compact once history exceeds it), `max_tokens` is the *output budget* (total tokens handed to the main LLM after compaction), `max_summary_tokens` *caps the summary itself* — so a sane ordering is summary < output < trigger (here 128 < 384 < 512). Trigger well below the model's window (context rot starts early, trap #8), and keep the summary cap small or the summary becomes the new bloat.
