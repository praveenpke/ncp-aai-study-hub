...
| **Answer relevance** | Does the *answer* actually address the *question*? RAGAS measures this by **reverse-engineering**: ask an LLM to *generate the question(s) the answer seems to be answering*, then cosine-compare those to the real question. *Intuition:* a focused, on-topic answer reconstructs the original question well (high similarity); an evasive or padded one reconstructs a different question (low similarity). | Evasive, off-topic, or incomplete answers |
...
| **NeMo Retriever NIMs** | The RAG-tuning levers as products. **Current family (NeMo Retriever NIM 1.13.0, the Nemotron rebrand):** embedding NIMs **`llama-nemotron-embed-1b-v2`** (8192-token, multilingual, Matryoshka dynamic-dim — was `llama-3.2-nv-embedqa-1b-v2`), **`llama-nemotron-embed-300m-v2`** (was `llama-3.2-nemoretriever-300m-embed-v2`), VL embed **`llama-nemotron-embed-vl-1b-v2`**; reranking **`llama-nemotron-rerank-1b-v2`** (+ VL `llama-nemotron-rerank-vl-1b-v2`). *Architecture detail still examinable* (the older NV-RerankQA-Mistral4B-v3 reranker): a LoRA-fine-tuned reranker based on Mistral-7B-v0.1 that keeps only the **first 16 of 32 transformer layers (~4 B params)** for throughput, switches attention from causal to bi-directional, and uses **mean-pooling over the last layer + a binary classification head** that emits a single relevance logit. (Older embed names still seen: NV-EmbedQA-E5-v5, NV-EmbedQA-Mistral7B-v2, Snowflake-Arctic-Embed-L.) | Adding the reranking NIM over a wider retrieval set improves accuracy *and* cuts cost (fewer, better chunks to the LLM) |
| **NAT Optimizer** (`nat optimize`) | The toolkit's **automated, eval-driven tuning** of a workflow's *config* — the rung between manual prompting and fine-tuning. Uses your `nat eval` evaluators as the objective and searches the config for the best settings | Optimizes **hyperparameters** (temperature, top_p, max_tokens — via **Optuna**), **prompts** (population evolved per generation with **LLM-powered mutation/recombination = a genetic algorithm**), and **model choices**. **Multi-objective:** plots pairwise **Pareto curves** across accuracy vs cost vs latency so you pick the trade-off point (NVIDIA's example: ~35% accuracy gain + ~20% token-efficiency gain). Install `nvidia-nat[config-optimizer]`. *Distinct from the Profiler* (which only *measures* per-step cost) — the Optimizer *changes* config to improve the eval score |
...
`nat sizing calc` estimates the **GPU cluster size** needed to serve a workflow at a target scale. It runs the workflow at several **concurrency levels**, uses the Profiler subsystem to measure throughput/latency, then **slope-based estimation** to extrapolate how performance scales with load. *What "slope-based" means:* it plots a measured metric (e.g., per-request latency, or runtime) against the number of concurrent users across the sampled concurrency points, **fits a line (the slope = how fast latency degrades per added user)**, and extrapolates that line out to `--target_users` to predict whether one GPU's worth of capacity will still hit your runtime target — if not, it scales the GPU count up until the projected point clears the target. (Linear extrapolation is why you give it several concurrency samples, not one.)
...
### 4.2b NAT Optimizer (`nat optimize`) — automated, eval-driven config tuning

This is the **"tuning" half of the domain done inside the toolkit**, and it's easy to miss: where the Profiler/Sizing Calculator only *measure*, the **Optimizer changes the workflow config to raise your eval score** — no GPU training loop, no labeled fine-tune data. It uses your existing `nat eval` evaluators (e.g., `ragas`, `trajectory`, accuracy) as the **objective function** and searches the config space:

- **What it tunes:** **hyperparameters** (temperature, top_p, max_tokens…), **prompts**, and **model choices** per node.
- **How it searches:** **Optuna** for numerical hyperparameters; a **genetic algorithm for prompts** — a *population* of prompt variants is evolved each generation via **LLM-powered mutation (+ optional recombination)**, kept if eval score improves.
- **Multi-objective:** it balances competing goals (e.g., **accuracy vs cost vs latency**) and plots **pairwise Pareto curves** so you choose the trade-off point — middle of the front for balance, the high-accuracy corner if accuracy is paramount. NVIDIA's worked example reports ~**35% accuracy improvement + ~20% token-efficiency gain** from optimized temperature/top_p/max_tokens.
- **Install/run:** `pip install nvidia-nat[config-optimizer]`, then `nat optimize` against the workflow config.

*Where it sits on the tuning ladder:* it's **automated prompt + parameter optimization** — strictly *above* hand-written prompting and *below* fine-tuning (no new weights, no training data). The exam separation among the three NAT "improve" tools: **Profiler = where does cost/latency go? · Sizing Calculator = how many GPUs for N users? · Optimizer = automatically find the config/prompt that maximizes eval score within a cost/latency budget.**
...
| Automatically tune prompts/temperature/model choice to max eval score within a cost/latency budget | NAT Optimizer (`nat optimize`) |
...
20. **Three NAT "improve" tools — don't swap them.** **Profiler** = *measures* where tokens/latency go. **Sizing Calculator** = *measures* the GPU count for N users at a latency target. **Optimizer (`nat optimize`)** = *changes* the config (prompts via genetic algorithm, hyperparameters via Optuna, model choice) to **maximize the eval score** along an accuracy/cost/latency **Pareto front** — no training data, no new weights. "Automatically improve my agent's prompts/settings against my eval without fine-tuning" → **Optimizer**, not Customizer (which trains weights) and not the Profiler (which only measures).
...
- NAT Optimizer (`nat optimize`, Optuna + genetic-algorithm prompt optimization, Pareto): https://docs.nvidia.com/nemo/agent-toolkit/latest/improve-workflows/optimizer.html
