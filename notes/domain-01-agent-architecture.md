...
|---|---|---|
| Origin | Anthropic, Nov 2024; open standard | Google, Apr 2025; donated to the Linux Foundation; v1.0 spec |
...
| Discovery | Client lists tools at connect time (`tools/list`) | **Agent Card** — JSON at `/.well-known/agent-card.json`: `name`, `description`, `url`, `capabilities` (streaming, pushNotifications), `skills`, `securitySchemes`, signature |
...
5. **MCP vs A2A inversion** — MCP = agent↔**tools/context** (client-server, tools/resources/prompts); A2A = **agent↔agent** (opaque peers, Agent Cards, Tasks). They're complementary; "replace MCP with A2A" is always wrong.
...
```python
from langgraph.prebuilt import create_react_agent      # 2025+: also exposed as create_agent in the `langchain` pkg
